...
$result = mysql_query("SELECT *FROM Schedules") or die(mysql_error());
...
if (mysql_num_rows($result) > 0) {
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        $schedule = array();
        $schedule["id"] = $row["Id"];
        $schedule["action"] = $row["action"];
		$schedule["time"] = $row["time"];
        $schedule["every"] = $row["every"];
...
else 
{
    // If no data is found
	$response["success"] = 0;
    $response["message"] = "No data on schedules found";
